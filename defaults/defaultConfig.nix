...
in
{
  plugins = with pkgs.vimPlugins; with pkgs.vitalityVimPlugins;  [
    ...
    # { plugin = copilot-vim; }
    # { plugin = cmp-copilot; }
    { plugin = cmp-buffer; } #FIXME: I'm 12 and what is this?
    { plugin = cmp-nvim-lsp; }
    { plugin = cmp-path; }
    # { plugin = cmp-tabnine; }
    ...
    { plugin = lsp_extensions-nvim; } #FIXME:
    ...
    { plugin = nvim-lightbulb; config = "autocmd CursorHold,CursorHoldI * lua require'nvim-lightbulb'.update_lightbulb()"; } # FIXME: https://github.com/kosayoda/nvim-lightbulb#prerequisites
    ...
    # { plugin = telescope-frecency-nvim; config = "lua require('telescope').load_extension('frecency')"; } #FIXME:
    # { plugin = telescope-github-nvim; config = "lua require('telescope').load_extension('gh')"; } #FIXME:
